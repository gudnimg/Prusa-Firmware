...
StepperMode stepper_mode; // Motor mode
...
void st_update_stepper_mode(StepperMode mode) {
  stepper_mode = mode;
  eeprom_update_byte_notify((uint8_t *)EEPROM_SILENT, (uint8_t)mode);
}
  ...
  if(check_endstops)
  {
    ...
    if ((out_bits & (1<<Z_AXIS)) != 0) // -direction
    {
      ...
      if (! check_z_endstop) {
          ...
          if ((stepper_mode == StepperMode::STEALTH) && !(tmc2130_sg_homing_axes_mask & 0x04))
...
void st_current_init() //Initialize Digipot Motor Current
{
    ...
    if((stepper_mode == StepperMode::NORMAL) || (farm_mode) ){
