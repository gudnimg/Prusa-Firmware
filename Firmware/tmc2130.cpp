...
static void tmc2130_XYZ_reg_init(uint8_t axis)
{
	...
	const bool isStealth = (stepper_mode == StepperMode::STEALTH);
	if (axis == Z_AXIS) {
...
void tmc2130_st_isr()
{
	if (stepper_mode == StepperMode::STEALTH || tmc2130_sg_stop_on_crash == false || tmc2130_sg_homing_axes_mask != 0)
		return;
...
void tmc2130_set_pwm_ampl(uint8_t axis, uint8_t pwm_ampl)
{
    ...
    if (((axis == X_AXIS) || (axis == Y_AXIS)) && (stepper_mode == StepperMode::STEALTH))
        tmc2130_wr(axis, TMC2130_REG_PWMCONF, pwmconf[axis].dw);
}
