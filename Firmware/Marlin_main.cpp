...
/*	if (mcu & 1) SERIAL_ECHOLNRPGM(MSG_POWERUP);
  ...
  stepper_mode = (StepperMode)eeprom_init_default_byte((uint8_t*)EEPROM_SILENT, (uint8_t)StepperMode::NORMAL);
...
void change_power_mode_live(uint8_t mode)
{
  // Wait for the planner queue to drain and for the stepper timer routine to reach an idle state.
		...
		stepper_mode = static_cast<StepperMode>(mode);
		update_mode_profile();
...
void force_high_power_mode(bool start_high_power_section) {
    ...
    if (stepper_mode == StepperMode::STEALTH) {
        ...
        change_power_mode_live((start_high_power_section == true) ? (uint8_t)StepperMode::NORMAL : (uint8_t)StepperMode::STEALTH);
    }
}
#endif //TMC2130
...
*/
    ...
    */
    ...
    case 915:
    {
    uint8_t newMode = (mcode_in_progress==914) ? (uint8_t)StepperMode::NORMAL : (uint8_t)StepperMode::STEALTH;
    if (code_seen('R'))
    {
    ...
    else if (code_seen('P'))
    {
        ...
        st_update_stepper_mode((StepperMode)newMode);
    }
    else if (code_seen('Q'))
    {
        printf_P(PSTR("%S: %S\n"), _O(MSG_MODE),
            stepper_mode == StepperMode::NORMAL ?
            _O(MSG_NORMAL) : _O(MSG_SILENT)
        );

    }
      if ((uint8_t)stepper_mode != newMode
#ifdef PSU_Delta
          || !bEnableForce_z
#endif
        )
  ...
  for (uint8_t i = X_AXIS, mask = X_AXIS_MASK; i < NUM_AXIS; i++, mask <<= 1) {
    if(code_seen(axis_codes[i]))
    {
      ...
      if (relative)
        destination[i] += current_position[i];
#if MOTHERBOARD == BOARD_RAMBO_MINI_1_0 || MOTHERBOARD == BOARD_RAMBO_MINI_1_3
	  if (i == Z_AXIS && stepper_mode == StepperMode::AUTO) update_currents();
#endif //MOTHERBOARD == BOARD_RAMBO_MINI_1_0 || MOTHERBOARD == BOARD_RAMBO_MINI_1_3
    }
...
#ifdef TMC2130
    if (stepper_mode == StepperMode::NORMAL && print_percent_done_normal <= 100)
    {
...
#ifdef TMC2130
    // EEPROM is intentionally not updated here, see enable_force_z()
    stepper_mode = StepperMode::STEALTH;
    update_mode_profile();
...
void enable_force_z()
{
    ...
    // mode recovering
#ifdef TMC2130
    // Revert back to the EEPROM setting
    stepper_mode = static_cast<StepperMode>(eeprom_read_byte((uint8_t*)EEPROM_SILENT));
    update_mode_profile();
    tmc2130_init(TMCInitParams(true, FarmOrUserECool()));
#endif // TMC2130
